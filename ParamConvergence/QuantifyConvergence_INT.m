...
% INT was formerly known as LP (for loop), so any tags finishing in LP
% correspond to INT
...
population_sizes = [10; 100; 1000; 10000; 100000; 1000000];
...
for param_idx = 1:num_params  % Skipping the first two parameters
    for class_idx = 1:num_classes
        % ---- MEANS ----
...
%% === INT-ONLY SUMMARY STATISTIC PLOTS ===
% This version saves plots for group 4 (LP) only, across all parameters.
% Each summary statistic (mean, median, variance) has its own folder.

% Ensure INT corresponds to index 4 in group_names = {'AD', 'NAD', 'TAD', 'INT'};
lp_idx = 4;  
lp_name = group_names{lp_idx};

%% === MEANS ===
output_folder = 'Mean_Plots_LP';
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

for param_idx = 1:16
    % Extract means for LP
    means_across_pops = squeeze(all_means(:, param_idx, lp_idx));
    ...
    fig_title = sprintf('Parameter: %s | Classification: %s', param_name, lp_name);
    fig_filename = sprintf('%s/Mean_%s_%s.png', output_folder, clean_param, lp_name);
    
    fig = figure('Visible', 'off', 'Name', fig_title, 'NumberTitle', 'off');
    
    % Subplot 1: Mean vs Population Size
    subplot(2,1,1);
    ...
    hold on; yline(2, '--r', 'Convergence Threshold (2%)'); hold off;
...
disp('LP mean plots saved successfully.');

%% === MEDIANS ===
output_folder = 'Median_Plots_LP';
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

for param_idx = 1:16
    medians_across_pops = squeeze(all_medians(:, param_idx, lp_idx));
    ...
    fig_title = sprintf('Parameter: %s | Classification: %s', param_name, lp_name);
    fig_filename = sprintf('%s/Median_%s_%s.png', output_folder, clean_param, lp_name);
...
disp('LP median plots saved successfully.');

%% === VARIANCES ===
output_folder = 'Variance_Plots_LP';
if ~exist(output_folder, 'dir')
    mkdir(output_folder);
end

for param_idx = 1:16
    variance_across_pops = squeeze(all_variances(:, param_idx, lp_idx));
    ...
    fig_title = sprintf('Parameter: %s | Classification: %s', param_name, lp_name);
    fig_filename = sprintf('%s/Variance_%s_%s.png', output_folder, clean_param, lp_name);
...
disp('LP variance plots saved successfully.');
